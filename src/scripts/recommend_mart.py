...
def main():
    ...
    geo_path = sys.argv[4]
    rec_mart_path = sys.argv[5]

    conf = SparkConf().setAppName(f"RecommendMart-{date}-d{days_count}")
    sc = SparkContext(conf=conf)
    sql = SQLContext(sc)

    end_date = F.to_date(F.lit(date), "yyyy-MM-dd")

    events_recommendations_mart = sql.read.parquet(events_path).filter(
    ...
    events_with_geo = add_coords_timezone(events_recommendations_mart, geo)
    users_distance_df = calculate_user_distance(events_with_geo)

    recs_dtmrt = calculate_recommendations_datamart(
        events_recommendations_mart,
        events_with_geo,
        users_distance_df,
    )

    recs_dtmrt.write.parquet(f"{rec_mart_path}/date={date}/depth={days_count}")
...
def calculate_recommendations_datamart(events_df, message_city_df,
                                       users_distance_df):
    subs = (
    ...
    subs_pairs = subs.join(
        subs.select(F.col("user_id").alias("user_right"),
                    "subscription_channel"),
        "subscription_channel",
    ).where("user_id < user_right")
    ...
    local_time_df = calculate_user_localtime(message_city_df)

    recommend_mart = (
        users_distance_df.join(subs_pairs, on=["user_id", "user_right"],
                               how="leftsemi")
